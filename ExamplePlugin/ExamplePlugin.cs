...
namespace ExamplePlugin
{
    ...
    public class FasterShrineOfTheMountain : BaseUnityPlugin
    {
        //The Plugin GUID should be a unique ID for this plugin, which is human readable (as it is used in places like the config).
        //If we see this PluginGUID as it is on thunderstore, we will deprecate this mod. Change the PluginAuthor and the PluginName !
        public const string PluginGUID = PluginAuthor + "." + PluginName;
        public const string PluginAuthor = "Petralka";
        public const string PluginName = "FasterShrineOfTheMountain";
        public const string PluginVersion = "1.1.0";
        public static ConfigEntry<bool> mountainShrine { get; set; }
...
#pragma warning restore Publicizer001
            ...
            mountainShrine = base.Config.Bind<bool>("Faster", "Mountain Shrine", true, "Should the mountain shrine be faster?");
            if (mountainShrine.Value)
            {
                On.RoR2.PurchaseInteraction.OnInteractionBegin += (orig, self, activator) =>
                {
                    ...
                    if (mountainShrine.Value)
                    {
                        ShrineBossBehavior behavior = self.GetComponent<ShrineBossBehavior>();
                        if (behavior != null)
                        {
